...
[[ "$HOST" != "ZH-MacBook-Air.local" ]] && prompt-set 10 "&gf(#55CDFC)%D{%a} &gf(#F7A8B8)%D{%m-}&f%D{%d} &gf(#F7A8B8)%D{%H:}&gf(#55CDFC)%D{%M}&r "
# Hostname
prompt-set 20 "&1%m&r "
[[ "$HOST" == "ZH-MacBook-Air.local" ]] && prompt-set 20 "&gf(#55CDFC)ZH&gf(#F7A8B8)NET&fD&gf(#F7A8B8)-ORGE&gf(#55CDFC)-Control&r "
